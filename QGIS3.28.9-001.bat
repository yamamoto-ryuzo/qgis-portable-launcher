    ...
    if exist "QGIS_internet.cfg" (
        ...
        for /f "tokens=1,2" %%a in (QGIS_plugin.txt) do (
            ...
            if not exist plugin\!QGIS_plugin_File! (
                ...
                bitsadmin /transfer download_QGIS_plugin_!QGIS_plugin_http! !QGIS_plugin_http!!QGIS_plugin_File! plugin\!QGIS_plugin_File!
            )
        )
    )
)
...
fc /L qgisconfig\system_ver\qgisconfig_ver.txt %USERPROFILE%\Documents\qgisconfig\system_ver\qgisconfig_ver.txt
...
if %errorlevel%==0 (
    ...
    call powershell -command "copy-item qgisconfig\profiles %USERPROFILE%\Documents\qgisconfig\profiles -Recurse -force

    rem "qgisconfig_ver.txtの配布"
    call powershell -command "copy-item qgisconfig\system_ver\qgisconfig_ver.txt %USERPROFILE%\Documents\qgisconfig\system_ver\qgisconfig_ver.txt -Recurse -force
)
...
for /f "tokens=1,2,3" %%a in (QGIS_plugin.txt) do (
    ...
    if not exist %USERPROFILE%\Documents\qgisconfig\profiles\default\python\plugins\!QGIS_plugin_folder! (
        ...
        rem "バージョンチェック"
        fc /L qgisconfig\system_ver\!QGIS_plugin_folder!_ver.txt %USERPROFILE%\Documents\qgisconfig\system_ver\!QGIS_plugin_folder!_ver.txt
            if %errorlevel%==0 (
            rem "ファイル内容が等しい"
            rem msg %username% !QGIS_plugin_folder!バージョンアップはありません
        ) else (
            rem "!QGIS_plugin_folder!_ver.txtの配布"
   	    call powershell -command "copy-item qgisconfig\system_ver\!QGIS_plugin_folder!_ver.txt %USERPROFILE%\Documents\qgisconfig\system_ver\!QGIS_plugin_folder!_ver.txt -Recurse -force

	    set plugins_flag=1            rem "ファイル内容が違う、ファイルがない等"
            rem "コマンドプロンプトはネットワークフォルダ非対応のためpowershellで実行"
            rem "フォルダ以下すべて -Recurse"
            rem "既存フォルダがあっても強制 -force" 
            rem "解凍の前にいったん削除"
            ...
            powershell Expand-Archive -Path  plugin\!QGIS_plugin_File! %USERPROFILE%\Documents\qgisconfig\profiles\default\python\plugins
